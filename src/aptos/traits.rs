    ...
    + private::Sealed
{
    ...
    fn verify_struct_signature(
        &self,
        bcs_msg: Vec<u8>,
        signature: &Self::SignatureMaterial,
    ) -> Result<()> {
        signature.verify(bcs_msg, self)
    }
    ...
    fn batch_verify<T: CryptoHash + Serialize>(
        ...
        Self::SignatureMaterial::batch_verify(bcs_msg, keys_and_signatures)
    }
}
    ...
    + private::Sealed
{
    ...
    fn verify(&self, bcs_msg: Vec<u8>, public_key: &Self::VerifyingKeyMaterial) -> Result<()>;
    ...
    fn batch_verify(
        ...
        for (key, signature) in keys_and_signatures {
            signature.verify(bcs_msg.to_vec(), &key)?
        }
        Ok(())
    }
}
...
pub trait SigningKey:
    PrivateKey<PublicKeyMaterial = <Self as SigningKey>::VerifyingKeyMaterial> + ValidCryptoMaterial
// + private::Sealed
{
    ...
    fn sign(&self, bcs_msg: Vec<u8>) -> Result<Self::SignatureMaterial, CryptoMaterialError>;
